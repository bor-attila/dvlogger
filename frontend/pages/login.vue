...
<template>
  <div class="flex min-h-screen items-center justify-center">
    <form class="w-80 space-y-4 rounded-lg border border-zinc-800 bg-zinc-900 p-6" @submit.prevent="submit">
      ...
      <input v-model="user" placeholder="Username" class="w-full rounded bg-zinc-800 px-3 py-2" autofocus />
      <input v-model="password" type="password" placeholder="Password" class="w-full rounded bg-zinc-800 px-3 py-2" />
      <p v-if="error" class="text-sm text-red-400">{{ error }}</p>
      <button class="w-full rounded bg-emerald-600 py-2 font-medium hover:bg-emerald-500">Log in</button>
    </form>
  </div>
</template>
